...
if __name__ == "__main__":
    ...
    # Add argument
    parser.add_argument('--data_root', type=str, default='/scratch/yangzd/materials/data/md/lips/20k', help='data directory', required=False)
    parser.add_argument('--model_dir', type=str, default='./wandb/run-20240623_084150-LEIGNN_LiPS_20240623_084148', help='model_dir such as ./wandb/run-20231031_144315-LEIGNN_20231031_144314/', required=False)
    parser.add_argument('--timestep', type=float, default=0.25, help='This defines the time interval for each step of the simulation, in femtoseconds (fs)')
    parser.add_argument('--temperature', type=float, default=520., help='This sets the target temperature for the Langevin thermostat, in Kelvin')
    parser.add_argument('--ttime', type=float, default=20, help='This parameter is specific to the Langevin integrator and controls the strength of the friction (or damping) term')
    parser.add_argument('--save_frequency', type=int, default=50, help='This parameter defines how often the data from the simulation is saved')
    parser.add_argument('--T_init', type=float, default=520., help='This parameter sets the initial temperature of the system at the start of the simulation, measured in Kelvin')
    parser.add_argument('--steps', type=int, default=200000, help='This is the total number of steps the simulation will run.')
    ...
    integrator = NoseHoover(atoms, timestep*units.fs, temperature=temperature*units.kB, ttime=ttime)
