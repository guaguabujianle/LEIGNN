...
class LEIGNNUpdate(nn.Module):
    def __init__(self, hidden_channels):
        ...
        self.global_x_proj = nn.Sequential(
            nn.Linear(hidden_channels, hidden_channels // 2),
            ScaledSiLU(),
            nn.Linear(hidden_channels // 2, 1),
        )
        self.global_vec_proj = nn.Linear(hidden_channels, 1, bias=False)

        self.inv_sqrt_2 = 1 / math.sqrt(2.0)
        self.inv_sqrt_h = 1 / math.sqrt(hidden_channels)

    def forward(self, x, vec, batch, natoms):
        vec1, vec2 = torch.split(
            self.vec_proj(vec), self.hidden_channels, dim=-1
        )

        vec_sq = self.global_vec_proj(vec)
        x_sq = self.global_x_proj(x).unsqueeze(1)

        global_vec = scatter(x_sq * vec_sq, index=batch, dim=0, dim_size=batch.max()+1, reduce='sum') 
        global_vec = global_vec.repeat_interleave(natoms, dim=0)
        vec_dot = (vec2 * global_vec).sum(1)
        vec_norm = vector_norm(vec2)

        x_vec_h = self.xvec_proj(
            torch.cat(
                [x, vec_dot, vec_norm], dim=-1
            )
        )
        xvec1, xvec2 = torch.split(
            x_vec_h * self.inv_sqrt_2, self.hidden_channels, dim=-1
        )

        dx = (self.x_proj(x) + xvec1) / 2
        dvec = xvec2.unsqueeze(1) * vec1
...
class LEIGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        for i in range(self.num_layers):
            ...
            dx, dvec = self.update_layers[i](x, vec, batch, natoms)
            x = x + dx
            vec = vec + dvec
