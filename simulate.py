...
if __name__ == "__main__":
    ...
    parser.add_argument('--timestep', type=int, default=0.5, help='This defines the time interval for each step of the simulation, in femtoseconds (fs)')
    parser.add_argument('--temperature_K', type=int, default=300, help='This sets the target temperature for the Langevin thermostat, in Kelvin')
    parser.add_argument('--friction', type=int, default=8, help='This parameter is specific to the Langevin integrator and controls the strength of the friction (or damping) term')
    ...
    parser.add_argument('--T_init', type=int, default=300, help='This parameter sets the initial temperature of the system at the start of the simulation, measured in Kelvin')
    ...
    meta_path = os.path.join(data_root, 'metadata.npy')
    mean = np.load(meta_path, allow_pickle=True).item()['e_mean']
    normalizer = RemoveMean(mean)

    device = torch.device('cuda:0')

    model = LEIGNN(hidden_channels=512, num_layers=4, num_rbf=128, cutoff=5.0, max_neighbors=50, use_pbc=True, otf_graph=True)
