...
if __name__ == "__main__":
    ...
    path = os.path.join(data_root, 'metadata.npy')
    mean = np.load(path, allow_pickle=True).item()['e_mean']
    normalizer = RemoveMean(mean)

    timestamp = time.strftime("%Y%m%d_%H%M%S")
    if systems == "CH4":
        log_name = f'LEIGNN_CH4_{timestamp}'
        # name is the name displayed in UI
        # id is the name displayed in local dir
        wandb.init(project="LEIGNN_CH4", 
                config={"train_len" : len(train_set), "valid_len" : len(valid_set)}, 
                name=log_name,
                id=log_name
                )
    elif systems == "H2O":
        log_name = f'LEIGNN_H2O_{timestamp}'
        # name is the name displayed in UI
        # id is the name displayed in local dir
        wandb.init(project="LEIGNN_H2O", 
                config={"train_len" : len(train_set), "valid_len" : len(valid_set)}, 
                name=log_name,
                id=log_name
                )  
    ...
    model = LEIGNN(hidden_channels=512, num_layers=4, num_rbf=128, cutoff=5.0, max_neighbors=50, use_pbc=True, otf_graph=True).to(device)
    optimizer = optim.AdamW(model.parameters(), lr=1e-3, weight_decay=0, amsgrad=True)
